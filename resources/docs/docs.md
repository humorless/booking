...
The `home-routes` handler in the `booking.routes.home` namespace
...
The `home-page` function will in turn call the `booking.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `booking.handler` namespace:
...
Request middleware functions are located under the `booking.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `booking.dev-middleware` namespace found in
